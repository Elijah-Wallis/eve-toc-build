...
class PostcallReconciler:
    def __init__(self, telemetry: Optional[Telemetry] = None) -> None:
        ...
        self.ontology = OntologyClient(telemetry=telemetry, actor="postcall-reconciler")
    ...
    def _fetch_lead_map(self, lead_ids: Iterable[Any]) -> Dict[str, Dict[str, Any]]:
        rows = self.ontology.list_patient_journeys_by_ids(
            lead_ids,
    ...
    def _patch_call_session_if_needed(self, session: Dict[str, Any], record: Dict[str, Any], *, force: bool) -> bool:
        ...
        if not patch:
            return False
        self.ontology.patch_clinical_encounter(str(session["id"]), patch)
        return True
    ...
    def _patch_lead_and_segment_if_needed(self, lead: Dict[str, Any], record: Dict[str, Any], *, force: bool) -> bool:
        ...
        self.ontology.patch_patient_journey(lead_id, patch)
        self.ontology.upsert_patient_journey_segment(
            lead_id,
            outcome,
    ...
    def _upsert_postcall_event(self, session: Dict[str, Any], lead: Dict[str, Any], retell_payload: Dict[str, Any]) -> bool:
        ...
        if not lead_id or not call_id:
            return False
        data = self.ontology.upsert_patient_journey_event(
            {
                "lead_id": lead_id,
    ...
    def _persist_validation_failure(self, failure: Any) -> None:
        ...
        self.ontology.persist_validation_failure(report_row, quarantine_row)

    def _upsert_transcript_and_turns(self, record: Dict[str, Any], turns: List[Dict[str, Any]]) -> None:
        row = self.ontology.upsert_clinical_transcript(record)
        ...
        self.ontology.replace_clinical_transcript_turns(
            str(transcript_id),
            str(record.get("retell_call_id") or ""),
            payload_rows,
        )
    ...
    def _transcript_tables_enabled(self) -> bool:
        ...
        self._transcript_tables_available = self.ontology.transcript_tables_enabled()
